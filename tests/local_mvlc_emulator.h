#ifndef FIDGET_TESTS_LOCAL_MVLC_EMULATOR_H
#define FIDGET_TESTS_LOCAL_MVLC_EMULATOR_H
...
inline std::uint32_t EncodeSigned14(const int value)
{
    return static_cast<std::uint32_t>(value) & 0x3FFFU;
}

inline std::uint32_t PackSamples(const int even, const int odd)
{
    return 0x30000000U | EncodeSigned14(even)
        | (EncodeSigned14(odd) << 14U);
}

inline std::vector<std::byte> MakeWaveformPacket(
...
inline std::vector<std::uint32_t> SuperFrame(
...
inline fidget::DiagnosticAcquisitionPreparationRequest MakeRequest(
...
inline void RunDiagnosticAcquisitionLifecycle(
    fidget::ICommandTransport& commandTransport,
    fidget::IDataReceiver& dataReceiver,
    LocalMvlcEmulator& emulator,
    const JournalPath& journal,
    const fidget::ScpCaptureOwnershipGate& ownershipGate)
{
    const auto request = MakeRequest(emulator, journal.Get());
    const std::atomic<bool> cancelled{false};
    auto prepared = fidget::PrepareDiagnosticAcquisition(
        commandTransport,
        request,
        cancelled,
        ownershipGate);
    ...
    fidget::DiagnosticStreamSnapshot published;
    fidget::AcquisitionReceiver receiver(
        dataReceiver, std::chrono::milliseconds(5));
    REQUIRE(receiver.Start(
        29U,
        [&](const fidget::DiagnosticStreamSnapshot& next) {
            {
    ...
    const auto fingerprint = fidget::VerifyDiagnosticOwnershipFingerprint(
        commandTransport,
        prepared,
        heartbeatReference,
        cancelled);
    ...
    CHECK(fingerprint.outcome
          == fidget::DiagnosticFingerprintOutcome::Verified);

    receiver.StopAndJoin();
    prepared.nextSuperReference = heartbeatReference;
    prepared = fidget::StopDiagnosticAcquisition(
        commandTransport,
        dataReceiver,
        std::move(prepared),
        request,
